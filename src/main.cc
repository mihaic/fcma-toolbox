...
static void params_from_keyvalues(char** keys_and_values,
                                  const int& num_elements);

void exit_with_help() {
  printf(
      "**********************************************************\n"
      "Usage 1: [OMP_NUM_THREADS=n mpirun -np n -hostfile host] ./pni_fcma -d "
      "matrix_directory -m matrix_file_type -t output_file -k taskType "
      "[options]\n"
      "Usage 2: [OMP_NUM_THREADS=n mpirun -np n -hostfile host] ./pni_fcma "
      "config.fcma\n"
      "required:\n"
      "-d nifti file directory, contains files in nifti gz format\n"
      "-1 one of the nifti file directories, if providing -1, -2 must be "
      "provided, too. providing -1 and -2 will cause error if providing -d as "
      "well\n"
      "-2 the other one of the nifti file directories, -1 and -2 MUST have the "
      "same number of nifti files and file name sequence should be matched. -2 "
      "won't be used in activity-based analysis\n"
      ...
      "-b block information file, if no block information file, a block "
      "information directory is required\n"
      ...
      "-l leave out id, the first block id that being left out, default -1, "
      "which means don't leave out anything\n"
      "-h number of items that held for test, default -1, only and must get "
      "values when -l is applied\n"
      ...
      "-y the second mask file, default no mask. for voxel selection, we "
      ...
      "random time seed (results are not repeatable), 2-shuffle using an input "
      "permuting book (results are repeatable), default 0\n"
      "-p permuting book, only get values when -f is set to 2; the book "
      "contains #subjects row and #voxels column; each row contains random "
      "shuffling result of 0 to #voxels-1\n");
  FATAL("usage");
}
...
void set_default_parameters() {
  ...
  Parameters.block_information_file = Parameters.block_information_directory =
      NULL;
  Parameters.step = 100;
...
void check_parameters() {
  ...
  if (Parameters.fmri_directory == NULL && Parameters.fmri_directory1 == NULL &&
      Parameters.fmri_directory2 == NULL) {
  ...
  if (Parameters.block_information_file == NULL &&
      Parameters.block_information_directory == NULL) {
  ...
  if (Parameters.output_file == NULL &&
      (Parameters.taskType == Corr_Based_SVM ||
       Parameters.taskType == Corr_Based_Dis ||
       Parameters.taskType == Acti_Based_SVM ||
       Parameters.taskType == Corr_Sum)) {
...
void parse_command_line(int argc, char** argv) {
  ...
  for (int i = start + 1; i < argc; i += 2)  // here i should increase by two,
                                             // but the code here is not safe
                                             // now
  {
    switch (argv[i - 1][1]) {
      case 's':
...
#define KEYS_DEF                                                        \
  KEY_DEF(FCMA_DATADIR, "datadir"),                                     \
      ...
      KEY_DEF(FCMA_NUM_FOLDS_IN_FEATURE_SELECTION,                      \
              "num_folds_in_feature_selection"),                        \
...
void run_fcma(Param* param) {
  ...
  MPI_Bcast((void*)&step, 1, MPI_INT, 0, MPI_COMM_WORLD);
  int nTrials = 0;
  int nSubs = 0;
...
#ifndef __MIC__
  if (me == 0) {
    pts = ReadLocInfoFromNii(r_matrices[0]);  // assume that all subjects are
                                              // aligned; use first subject's
                                              // voxel coordinates
  }
#endif
  Trial* trials = NULL;
  ...
  if (taskType == Acti_Based_SVM || taskType == Acti_Mask_Classification ||
      taskType == Acti_Mask_Cross_Validation) {
    int nBlocksPerSub =
        nTrials / nSubs;  // assume each subject has the same number of blocks
    avg_matrices =
        rawMatPreprocessing(r_matrices, nSubs, nBlocksPerSub,
                            trials);  // 18 subjects, 12 blocks per subject
  }
  if (taskType != Corr_Mask_Cross_Validation &&
      taskType != Acti_Mask_Cross_Validation &&
      leave_out_id >= 0)  // k==5 or 7 implies a cross validation
  {
    leaveSomeTrialsOut(trials, nTrials, leave_out_id,
                       nHolds);  // leave nHolds blocks out every time, usually
                                 // 1 entire subject
  }
  if (param->isTestMode &&
      me == 0)  // program in test mode only uses one node to predict
  {
    ...
    if (mask_file1 != NULL) {
      ...
      if (mask_file2 != NULL) cout << mask_file2;
      cout << endl;
    }
    ...
    switch (taskType) {
      ...
      case Acti_Based_SVM:
      case Corr_Sum:
        SVMPredict(r_matrices, r_matrices2, avg_matrices, nSubs, nTrials,
                   trials, nHolds, taskType, output_file, mask_file1,
                   is_quiet_mode);
        break;
      ...
      case Acti_Mask_Classification:
        result = SVMPredictActivationWithMasks(avg_matrices, nSubs, mask_file1,
                                               nTrials, trials, nHolds,
                                               is_quiet_mode);
        cout << "accuracy: " << result << "/" << nHolds << "="
             << result * 1.0 / nHolds << endl;
        break;
      case Acti_Mask_Cross_Validation:
        ...
        while (l <= nTrials - nHolds)  // assume that nHolds*an integer==nTrials
        {
          leaveSomeTrialsOut(trials, nTrials, 0, nHolds);  // the setting of
                                                           // third parameter is
                                                           // tricky here
          int curResult = SVMPredictActivationWithMasks(
              avg_matrices, nSubs, mask_file1, nTrials, trials, nHolds,
              is_quiet_mode);
          f++;
          cout << "fold " << f << ": " << curResult << "/" << nHolds << "="
               << curResult * 1.0 / nHolds << endl;
  ...
  } else if (!param->isTestMode)  //  program not in test mode is in voxel
                                  // selction mode, which will use multiple
                                  // nodes if doing correlation based selection
  {
    ...
    switch (taskType) {
      ...
      case Corr_Sum:
        if (me == 0)  // master process doesn't need to keep r_matrices
        {
          ...
          if (taskType == Corr_Based_Dis)
            cout << "distance ratio selecting..." << endl;
          if (taskType == Corr_Sum) cout << "correlation sum..." << endl;
        }
        Scheduler(me, nprocs, step, r_matrices, r_matrices2, taskType, trials,
                  nTrials, nHolds, nSubs, nFolds, output_file, mask_file1,
                  mask_file2, shuffle, permute_book_file);
        break;
      case Acti_Based_SVM:
        cout << "Searchlight selecting..." << endl;
        Searchlight(avg_matrices, nSubs, trials, nTrials, nHolds, nFolds, pts,
                    output_file, mask_file1, shuffle,
                    permute_book_file);  // doesn't need mpi
        break;
      case Marginal_Screening:
        if (me == 0) {
          cout << "marginal screening starting..." << endl;
        }
        Marginal_Scheduler(me, nprocs, step, r_matrices, taskType, trials,
                           nTrials, nHolds, nSubs, nFolds, output_file,
                           mask_file1);
        break;
      default:
  ...
  if (me == 0) {
    cout.precision(6);
    cout << "it takes " << tstop - tstart
         << " s to compute the whole task (exclude data reading but include "
            "data transferring using MPI)" << endl;
  }
  /* main program ends */
  /* -------------------------------- */
  /* Shut down MPI */
